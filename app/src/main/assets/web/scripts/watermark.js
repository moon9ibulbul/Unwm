...
function loadImage() {
    ...
    image.onload = async function () {
            ...
            document.dispatchEvent(new CustomEvent('astral:image-loaded', {
                detail: {
                    index: currentImage,
...
async function applyWatermarkImage(wm_img_load, sourceLabel) {
    ...
    ctx.drawImage(wm_img_load, 0, 0)
    wm_ib = await createImageBitmap(wm_img_load, {
        colorSpaceConversion: 'none',
        premultiplyAlpha: 'none',
        resizeQuality: 'pixelated'
    })
    ...
    watermark.style.width = String(scale_x * wm_img_load.naturalWidth) + 'px'
    watermark.style.height = String(scale_y * wm_img_load.naturalHeight) + 'px'
    watermark.setAttribute('data-source', sourceLabel || '')
    updateWatermarkPosition()
    document.dispatchEvent(new CustomEvent('astral:watermark-ready', {
        detail: {
    ...
    var autoToggle = document.getElementById('auto-detect-toggle')
    if (autoToggle && autoToggle.checked && typeof autoDetectWatermarkPosition === 'function') {
        setTimeout(() => {
            autoDetectWatermarkPosition().catch(err => console.warn('Auto detect failed', err))
        }, 10)
    }
}
function loadWatermark() {
    if(scale_x && scale_y) {
        ...
        wm_img_load.onload = function () {
            applyWatermarkImage(wm_img_load)
        }
...
function loadWatermarkFromDataUrl(dataUrl, label) {
    if (!(scale_x && scale_y)) {
        alert('Load image first.')
        return
    }
    let wm_img_load = document.createElement('img')
    wm_img_load.onload = function () {
        applyWatermarkImage(wm_img_load, label)
    }
    wm_img_load.src = dataUrl
}
window.loadWatermarkFromDataUrl = loadWatermarkFromDataUrl
function updateWatermarkPosition() {
    watermark.style.top = String(wm_y * scale_y) + 'px'
    watermark.style.left = String(wm_x * scale_x) + 'px'
}
function updateWatermarkStyle() {
    watermark.style.width = String(watermark.width * scale_x) + 'px'
    watermark.style.height = String(watermark.height * scale_y) + 'px'
    updateWatermarkPosition()
}
...
function triggerDownload(url) {
    ...
    document.dispatchEvent(new CustomEvent('astral:preview-ready', {
        detail: {
            before: originalUrl,
            after: url,
            name: files && files[currentImage] ? files[currentImage].name : ''
...
async function autoDetectWatermarkPosition() {
    ...
    try {
        if (!image_ib || !wm_ib) {
            throw new Error('Image atau watermark belum dimuat')
        }
        const maxDimension = 512
        const detectionScale = Math.min(1, maxDimension / Math.max(image_ib.width, image_ib.height, wm_ib.width, wm_ib.height))
        const scaledWidth = Math.max(1, Math.round(image_ib.width * detectionScale))
        const scaledHeight = Math.max(1, Math.round(image_ib.height * detectionScale))
        const wmScaledWidth = Math.max(1, Math.round(wm_ib.width * detectionScale))
        const wmScaledHeight = Math.max(1, Math.round(wm_ib.height * detectionScale))
        if (wmScaledWidth > scaledWidth || wmScaledHeight > scaledHeight) {
            throw new Error('Watermark lebih besar dari gambar')
        }
        ...
        imgCtx.drawImage(image_ib, 0, 0, scaledWidth, scaledHeight)
        const imgPixels = imgCtx.getImageData(0, 0, scaledWidth, scaledHeight).data
        const gray = new Float32Array(scaledWidth * scaledHeight)
        for (let i = 0, j = 0; i < imgPixels.length; i += 4, j++) {
            gray[j] = 0.2126 * imgPixels[i] + 0.7152 * imgPixels[i + 1] + 0.0722 * imgPixels[i + 2]
        }
        const edgeMap = new Float32Array(scaledWidth * scaledHeight)
        let maxEdge = 0
        for (let y = 0; y < scaledHeight; y++) {
            for (let x = 0; x < scaledWidth; x++) {
                const idx = y * scaledWidth + x
                const gx = (x + 1 < scaledWidth ? gray[idx + 1] : gray[idx]) - gray[idx]
                const gy = (y + 1 < scaledHeight ? gray[idx + scaledWidth] : gray[idx]) - gray[idx]
        ...
        wmCanvas.width = wmScaledWidth
        wmCanvas.height = wmScaledHeight
        const wmCtx = wmCanvas.getContext('2d')
        wmCtx.imageSmoothingEnabled = false
        wmCtx.drawImage(wm_ib, 0, 0, wmScaledWidth, wmScaledHeight)
        const wmPixels = wmCtx.getImageData(0, 0, wmScaledWidth, wmScaledHeight).data
        const edgePoints = []
        for (let y = 0; y < wmScaledHeight; y++) {
            for (let x = 0; x < wmScaledWidth; x++) {
                const idx = (y * wmScaledWidth + x) * 4 + 3
                const alpha = wmPixels[idx]
                if (alpha < 8) {
                    continue
                }
                const alphaRight = x + 1 < wmScaledWidth ? wmPixels[idx + 4] : alpha
                const alphaBottom = y + 1 < wmScaledHeight ? wmPixels[idx + wmScaledWidth * 4] : alpha
                const diff = Math.max(Math.abs(alpha - alphaRight), Math.abs(alpha - alphaBottom))
                if (diff > 12 || alpha > 160) {
                    edgePoints.push({
                        x: x,
                        y: y,
                        weight: Math.max(diff, alpha) / 255
                    })
                }
            }
        }
        if (!edgePoints.length) {
            throw new Error('Tidak menemukan pola watermark untuk deteksi otomatis')
        }

        const step = Math.max(1, Math.floor(Math.min(scaledWidth, scaledHeight) / 96))
        function scoreAt(px, py) {
            let score = 0
            for (let i = 0; i < edgePoints.length; i++) {
                const pt = edgePoints[i]
                const idx = (py + pt.y) * scaledWidth + (px + pt.x)
                score += edgeMap[idx] * pt.weight
            }
            return score / edgePoints.length
        }

        let best = { score: -Infinity, x: 0, y: 0 }
        for (let y = 0; y <= scaledHeight - wmScaledHeight; y += step) {
            for (let x = 0; x <= scaledWidth - wmScaledWidth; x += step) {
        ...
        for (let y = Math.max(0, best.y - refineRadius); y <= Math.min(scaledHeight - wmScaledHeight, best.y + refineRadius); y++) {
            for (let x = Math.max(0, best.x - refineRadius); x <= Math.min(scaledWidth - wmScaledWidth, best.x + refineRadius); x++) {
                const score = scoreAt(x, y)
                if (score > best.score) {
                    best = { score: score, x: x, y: y }
                }
        ...
        wm_x = best.x / detectionScale
        wm_y = best.y / detectionScale
        updateWatermarkPosition()
        document.dispatchEvent(new CustomEvent('astral:autodetect-end', {
            detail: {
                success: true,
                x: wm_x,
                y: wm_y
...
window.autoDetectWatermarkPosition = autoDetectWatermarkPosition
function nextImage() {
    currentImage++;
    if (files.length > 1) {
...
function resizeWatermark(w, h) {
    if (!hasBeenResized) {
        //make a backup of the watermark in case you miscalculated the width
        //and need to resize again
        hasBeenResized = true
        watermark_backup.width = watermark.width
        watermark_backup.height = watermark.height
        w_b_ctx = watermark_backup.getContext('2d')
        w_b_ctx.drawImage(watermark, 0, 0)
    }
    watermark.width = Math.round(w)
    watermark.height = Math.round(h)
    resizer.resize(watermark_backup, watermark, {
        alpha: true,
        filter: 'lanczos3'
    }).then(
        () => updateWatermarkStyle()
