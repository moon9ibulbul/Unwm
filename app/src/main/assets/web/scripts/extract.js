...
async function forwardExtractedSample(url) {
    if (!(window.AstralSamples && typeof window.AstralSamples.saveSample === 'function')) {
        return
    }
    try {
        let dataUrl
        ...
        const defaultName = 'Extracted watermark'
        const name = (typeof prompt === 'function' ? prompt('Nama sample watermark', defaultName) : defaultName) || defaultName
        window.AstralSamples.saveSample(name, dataUrl)
        if (window.AstralSamplesBridge && typeof window.AstralSamplesBridge.onSampleSaved === 'function') {
            window.AstralSamplesBridge.onSampleSaved(JSON.stringify({ id: '', name: name }))
        }
    } catch (err) {
        if (window.AstralSamples && typeof window.AstralSamples.showToast === 'function') {
            window.AstralSamples.showToast('Gagal mengirim sample: ' + err.message)
        }
    }
}
